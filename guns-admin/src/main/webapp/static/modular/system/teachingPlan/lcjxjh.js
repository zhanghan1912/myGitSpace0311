...
TeachingPlan.initColumn = function () {
    return [
        ...
        {title: '理论教学类型', field: 'activityName', visible: true, align: 'center', valign: 'middle'},
        ...
        {title: '上课班级', field: 'className', visible: true, align: 'center', valign: 'middle'}
    ];
};
...
TeachingPlan.openAddTeachingPlan = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/admin/lcjxjh_add'
    });
...
TeachingPlan.openTeachingPlanDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/admin/lcjxjh_update/' + TeachingPlan.seItem.id
        });
        this.layerIndex = index;
    }
};
...
TeachingPlan.search = function () {
    ...
    var table = new BSTable(TeachingPlan.id, "/admin/lcjxjh/list", defaultColunms);
...
TeachingPlan.openImportTeachingPlan = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/admin/lcjxjh_import'
    });
    this.layerIndex = index;
};
$(function () {
