...
TeachingPlan.initColumn = function () {
    return [
        ...
        {title: '全科轮转类型', field: 'activity_name', visible: true, align: 'center', valign: 'middle'},
...
TeachingPlan.joinPlan = function () {
    if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/visitor/lzddjh/joinPlan", function (data) {
            if(data.code==-1){
...
TeachingPlan.search = function () {
    ...
    var table = new BSTable(TeachingPlan.id, "/visitor/lzddjh/list", defaultColunms);
