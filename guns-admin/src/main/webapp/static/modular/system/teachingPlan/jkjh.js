...
TeachingPlan.initColumn = function () {
    return [
        ...
        {title: '监考考核类型', field: 'activityName', visible: true, align: 'center', valign: 'middle'},
...
TeachingPlan.openAddTeachingPlan = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/admin/jkjh_add'
    });
...
TeachingPlan.openTeachingPlanDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/admin/jkjh_update/' + TeachingPlan.seItem.id
        });
        this.layerIndex = index;
    }
};
...
TeachingPlan.search = function () {
    ...
    var table = new BSTable(TeachingPlan.id, "/admin/jkjh/list", defaultColunms);
...
TeachingPlan.openImportTeachingPlan = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/admin/jkjh_import'
    });
    this.layerIndex = index;
};
$(function () {
