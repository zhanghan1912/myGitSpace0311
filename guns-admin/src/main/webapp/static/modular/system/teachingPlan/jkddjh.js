...
TeachingPlan.initColumn = function () {
    return [
        ...
        {title: '监考考核类型', field: 'activity_name', visible: true, align: 'center', valign: 'middle'},
...
TeachingPlan.joinPlan = function () {
    if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/visitor/khddjh/joinPlan", function (data) {
            if(data.code==-1){
...
TeachingPlan.search = function () {
    ...
    var table = new BSTable(TeachingPlan.id, "/visitor/khddjh/list", defaultColunms);
