...
TeachingPlan.initColumn = function () {
    return [
        ...
        {title: '轮转督导类型', field: 'activityName', visible: true, align: 'center', valign: 'middle'},
...
TeachingPlan.openAddTeachingPlan = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/admin/lzjh_add'
    });
...
TeachingPlan.openTeachingPlanDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/admin/lzjh_update/' + TeachingPlan.seItem.id
        });
        this.layerIndex = index;
    }
};
...
TeachingPlan.search = function () {
    ...
    var table = new BSTable(TeachingPlan.id, "/admin/lzjh/list", defaultColunms);
...
TeachingPlan.openImportTeachingPlan = function () {
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/admin/lzjh_import'
    });
    this.layerIndex = index;
};
$(function () {
